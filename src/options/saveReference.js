...
function saveReferenceActiv(reference, desc) {
  db.all('SELECT activity_id, name FROM activity', async (err, rows_act) => {
    db.all('SELECT * FROM type_reference', async (err, rows_type_ref) => {
      const options = desc
        ? [
            {
              type: 'list',
      ...
      await prompt(options).then(res => {
        ...
        db.all('SELECT MAX(reference_id) as id FROM reference', (err, rows) => {
